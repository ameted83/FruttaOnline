  ...
  return (
    <div className="max-w-[1100px] mx-auto py-8 w-full my-20 ">
        <div className="mb-4 flex justify-center items-center" >
            <input className=" rounded-lg p-2 w-[90%] border-0 mt-2 mb-4 outline-0 text-lg bg-slate-100" onChange={(event)=>setSearch(event.target.value)} type="search" id="search" placeholder="Cerca il tuo frutto..." ></input>
        </div>
      <div className="relative grid mx-auto justify-center items-center w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
       ...
       {product.filter((fruit)=>{
        ...
        .map((fruit, index) => (
          <div key={index} className="flex flex-col mx-auto justify-center items-center w-[250px] ">
